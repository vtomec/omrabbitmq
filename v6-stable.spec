...
Source0: %{name}-%{version}-amqp.tar.gz
...
# NOT NEEDED ANYMORE Patch0: Patch0: rsyslog-6.4.0-systemd.patch
# already patched 
...
%if %{?_with_mysql:1}%{!?_with_mysql:0}
%package mysql
...
%if %{?_with_gssapi:1}%{!?_with_gssapi:0}
%package gssapi
...
%if %{?_with_gnutls:1}%{!?_with_gnutls:0}
%package gnutls
...
%if %{?_with_udpspoof:1}%{!?_with_udpspoof:0}
%package udpspoof
...
%if %{?_with_rabbitmq:1}%{!?_with_rabbitmq:0}
%package rabbitmq
Summary: Provides the omrabbitmq module
Group: System Environment/Daemons
Requires: %name = %version-%release
...
The rsyslog-rabbitmq package contains a dynamic shared object that will add
RabbitMQ support to rsyslog.
%endif
